...
These elements are described in different folders. All the modelling happens within the `openfisca_gme` folder.
...
```sh
git clone https://github.com/openfisca/openfisca-gme.git
cd openfisca-gme
...
Substitute your package's country name for `openfisca_gme` below:

```sh
curl -X POST -H "Content-Type: application/json" \
  -d @./openfisca_gme/situation_examples/couple.json \
